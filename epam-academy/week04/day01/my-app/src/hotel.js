...
class Left extends React.Component{
    render(){
        ...
        <div>
        <h4>Star Rating</h4>
        {this.props.filters['Star rating'].map((item) =>(
            <div key = {item.name}>
        ...
        <div>
            <h4>Property type</h4>
            {this.props.filters['Property type'].map((item) => (
                <div key={item.name}>
                    <input  checked={item.checked} type = 'checkbox'></input>
                    {item.name}
                </div>
...
class Right extends React.Component{
    ...
    return (
        <div>
            <h2>Hotel List</h2>
            {this.props.hotels.map(item => {return (<div key = {item.name}>
                <h3>{item.name}</h3>
    ...
    render(){
        let data = {
          ...
          return <div style = {style}>
              <Left filters = {data.filters} />
              <Right hotels = {data.hotels} />
          </div>
    }
}
